...
for image in image_path:
    ...
    for i in range(len(a[0])):
        Blue_trainingData[0].append(img_tcrcb[a[0][i],a[1][i],0])
        Blue_trainingData[1].append(img_tcrcb[a[0][i],a[1][i],1])
        Blue_trainingData[2].append(img_tcrcb[a[0][i],a[1][i],2])
file = open("BlueData","wb")
pickle.dump(Blue_trainingData,file)
file.close()
